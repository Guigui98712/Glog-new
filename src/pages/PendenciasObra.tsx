...
const PendenciasObra = () => {
  ...
  const etiquetas = [
    ...
    { nome: "Feito", cor: "bg-green-500 text-white" },
  ...
  const renderizarLabels = (labels: any) => {
    ...
    return labels.map((label: any, idx: number) => {
      ...
      return (
        <Badge 
          key={idx} 
          className={`text-xs ${getEtiquetaCor(labelText)}`}
        >
          {labelText}
        </Badge>
      );
  ...
  const handleToggleLabelInView = async (label: string) => {
    ...
    try {
      ...
      // Atualizar o card atual com as novas etiquetas
      const quadro = await obterQuadroObra(Number(id));
      const lista = quadro.lists.find(l => l.id === cardAtual.list_id);
      if (lista) {
        const card = lista.cards.find(c => c.id === cardAtual.id);
        if (card) {
          ...
          if (newBoard && newBoard.lists) {
            ...
            if (listaIndex !== -1) {
              const cardIndex = newBoard.lists[listaIndex].cards.findIndex(c => c.id === cardAtual.id);
              if (cardIndex !== -1) {
                // Converter strings para objetos TrelloLabel para satisfazer o tipo
                const labelObjects = labels.map(labelText => ({
                  id: 0, // ID temporário
                  title: labelText,
                  color: getEtiquetaCor(labelText)
                })) as TrelloLabel[];
                
                newBoard.lists[listaIndex].cards[cardIndex].labels = labelObjects;
                setBoard(newBoard);
              }
            }
          }
      ...
      toast({
        title: "Sucesso",
        description: "Etiquetas atualizadas com sucesso!",
      });
    } catch (error) {
            ...
            <div className="space-y-2">
              ...
              <div className="flex flex-wrap gap-2">
                {etiquetas.map((etiqueta) => (
                  <Badge 
                    key={etiqueta.nome}
                    variant={novoCard.labels.includes(etiqueta.nome) ? "default" : "outline"}
                    className={`cursor-pointer ${novoCard.labels.includes(etiqueta.nome) ? etiqueta.cor : ""}`}
                    onClick={() => handleToggleLabel(etiqueta.nome)}
                  >
                    {etiqueta.nome}
                  </Badge>
                ))}
